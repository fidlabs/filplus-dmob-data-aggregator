...
#[tracing::instrument(skip(source_db, dest_db))]
pub async fn process_provider_distributions(source_db: &SourceDatabase, dest_db: &DestDatabase) -> Result<()> {
    ...
    dest_db
        .begin().await?
        .write_provider_distributions(data).await?
        .commit().await?;
...
#[tracing::instrument(skip(source_db, dest_db))]
pub async fn process_replica_distributions(source_db: &SourceDatabase, dest_db: &DestDatabase) -> Result<()> {
    ...
    dest_db
        .begin().await?
        .write_replica_distributions(data).await?
        .commit().await?;
...
#[tracing::instrument(skip(source_db, dest_db))]
pub async fn process_cid_sharings(source_db: &SourceDatabase, dest_db: &DestDatabase) -> Result<()> {
